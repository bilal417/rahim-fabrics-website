 ...
 return <section className="section pt-10"><div className="mx-auto max-w-[1320px]">
  ...
  <div className="grid gap-12 lg:grid-cols-[1.08fr_.92fr]">
   <div className="grid gap-3 sm:grid-cols-[1fr_110px]">
    <div className={`${uploadedPhoto?'bg-cover bg-center':'fabric-tile'} min-h-[540px] bg-[#eee4d3]`} style={uploadedPhoto?{backgroundImage:`url(${photo})`}:{backgroundPosition:product.tilePosition||'center'}}/>
    <div className="grid grid-cols-3 gap-3 sm:grid-cols-1">{urls.length>1?urls.map((url,i)=><button aria-label={`View image ${i+1}`} key={url} onClick={()=>setActiveImage(i)} className={`min-h-28 bg-cover bg-center ${i===activeImage?'border-2 border-gold-500':''}`} style={{backgroundImage:`url(${url})`}}/>):<><div className="fabric-tile min-h-28 border-2 border-gold-500" style={{backgroundPosition:product.tilePosition||'center'}}/><div className="min-h-28 bg-[url('/images/showroom-hero.webp')] bg-cover bg-center"/><div className="fabric-tile min-h-28 [background-size:150%]" style={{backgroundPosition:product.tilePosition||'center'}}/></>}</div>
   </div>
   <div className="lg:pl-8"><p className="eyebrow">{product.category} · {product.code}</p><h1 className="mt-4 font-display text-4xl font-semibold leading-tight text-emerald-950 md:text-5xl">{product.name}</h1><p className="mt-6 leading-8 text-black/55">{product.description}</p>
    <div className="mt-8 border-y border-black/10 py-6"><div className="grid grid-cols-2 gap-5"><Spec icon={<Ruler/>} label="Thaan length" value={product.thaanLength}/><Spec icon={<PackageCheck/>} label="Packing" value={`1 Thaan · ${product.suitsPerThaan} suits`}/><Spec icon={<Check/>} label="Availability" value={product.stock>0?`${product.stock} thaans in stock`:'Confirm on inquiry'}/><Spec icon={<Share2/>} label="Fabric" value={product.fabricType}/></div></div>
    <div className="mt-7"><p className="text-xs font-bold uppercase tracking-widest text-black/45">Available colours</p><div className="mt-4 flex flex-wrap gap-2">{product.colors.map(c=><span key={c} className="rounded-full border border-black/10 bg-white px-3 py-2 text-xs font-semibold">{c}</span>)}</div></div>
